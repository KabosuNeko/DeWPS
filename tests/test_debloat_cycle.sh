...
# --- fake install ---
mkdir -p "$ADDONS_DIR/knewdocs" "$ADDONS_DIR/kwpscopilot" "$ADDONS_DIR/kdocerresource"
echo old > "$ADDONS_DIR/knewdocs/libcef.so"
...
check "fresh: knewdocs renamed"          yes "$( [[ -d $ADDONS_DIR/knewdocs.disabled && ! -e $ADDONS_DIR/knewdocs ]] && echo yes || echo no )"
...
# --- run 2: idempotent ---
cmd_debloat >/dev/null
check "idempotent: no nesting"      no "$( [[ -e $ADDONS_DIR/knewdocs.disabled/knewdocs ]] && echo yes || echo no )"
check "idempotent: stub intact"     yes "$(grep -q 'Disabled by DeWPS' "$OFFICE_DIR/wpsd" && echo yes || echo no)"

# --- simulate pacman update ---
mkdir -p "$ADDONS_DIR/knewdocs"
echo new > "$ADDONS_DIR/knewdocs/libcef.so"
echo new-binary > "$OFFICE_DIR/wpsd"
cmd_debloat >/dev/null
check "update: fresh addon kept as disabled" new "$(cat "$ADDONS_DIR/knewdocs.disabled/libcef.so" 2>/dev/null || echo missing)"
check "update: no nested dir"       no "$( [[ -e $ADDONS_DIR/knewdocs.disabled/knewdocs ]] && echo yes || echo no )"
...
mkdir -p "$ADDONS_DIR/knewdocs"
echo freshest > "$ADDONS_DIR/knewdocs/libcef.so"
echo freshest-binary > "$OFFICE_DIR/wpsd"
cmd_restore >/dev/null
check "restore-after-update: fresh addon kept"   freshest "$(cat "$ADDONS_DIR/knewdocs/libcef.so")"
check "restore-after-update: no nested junk"     no "$( [[ -e $ADDONS_DIR/knewdocs/knewdocs.disabled ]] && echo yes || echo no )"
...
# --- group selection ---
rm -rf "$ADDONS_DIR"
mkdir -p "$ADDONS_DIR"/{kfeedback,kskincenter,knewdocs,kwpscopilot}
for n in kfeedback kskincenter knewdocs kwpscopilot; do echo x > "$ADDONS_DIR/$n/f"; done
...
check "group: cloud kept"         no  "$( [[ -e $ADDONS_DIR/knewdocs.disabled ]] && echo yes || echo no )"
